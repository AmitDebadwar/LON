    ...
    public class LONServices : System.Web.Services.WebService
    {
        ...
        public string GetAllProductsUser(string data)
        {
            try
            {
                ...
                return new JavaScriptSerializer().Serialize(new LONDataClassesDataContext().ProductMasters.Select(c => new { c.ParentCategoryId, c.ProductDescription, c.ProductName, c.ProductPrice, c.ProductStatusId, c.ProductThumbnailImageFileName,c.ProductId }));
            }
            catch (Exception exp)
            {
